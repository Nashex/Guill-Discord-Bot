...
public class ProxyHelper {
    ...
    public ProxyHelper() {
        ...
        Guill.TIMER.scheduleWithFixedDelay(() -> {
        ...
        Guill.TIMER.scheduleWithFixedDelay(this::nextProxy, 2L, switchIpInterval, TimeUnit.SECONDS);
    }

    public void refreshProxyList() {
        LinkedHashMap<String, Integer> proxyList = new LinkedHashMap<>();
        try {
            Document doc = Jsoup.connect("https://proxy.webshare.io/api/proxy/list/?page=0&countries=US").header("Authorization" , "Token " + Guill.config.proxyToken)
...
//                        .timeout(3000)
                ...
                } catch (Exception e) {System.out.println("Invalid Proxy: " + currentProxy.getKey() + ":" + currentProxy.getValue()); timesTried++;}
            }
    ...
    public static Map.Entry<String, Integer> randomProxy(LinkedHashMap<String, Integer> proxyList) {
        if (ProxyHelper.proxyList.isEmpty()) Guill.proxyHelper.refreshProxyList();
