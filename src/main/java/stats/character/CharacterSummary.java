    ...
    public List<String> stringify(int limit) {
        ...
        for (Character character : characters) {
            ...
            if (currentString.length() + charString.length() < limit) {
                currentString.append(charString).append("\n");
            } else {
                list.add(currentString.toString());
                currentString = new StringBuilder(charString + "\n");
            }
        }
        if (list.isEmpty() && (currentString.length() == 0)) currentString = new StringBuilder("Player either has a private profile or no characters.");
        if (currentString.length() > 0) list.add(currentString.toString());
        return list;
    }
