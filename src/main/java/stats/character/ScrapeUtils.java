...
import data.LogField;
import main.Guill;
import org.jsoup.Jsoup;
import org.jsoup.nodes.Document;
import org.jsoup.nodes.Element;
import org.jsoup.select.Elements;
import utils.requests.RequestHub;
import utils.requests.SSLHelper;
...
public class ScrapeUtils {
    public static String petScrape(String playerName) {
        ...
        try {
            Document doc = Jsoup.connect("https://www.realmeye.com/pets-of/" + playerName)
                    .proxy(Guill.proxyHelper.currentProxy.getKey(), Guill.proxyHelper.currentProxy.getValue())
                    .headers(RequestHub.headers)
    ...
    public static String rankScrape(String playerName) {
        ...
        try {
            ...
            //TODO Add star emotes
//            if (rank < 16) {
//                rankHistory += Guill.jda.getEmoteById("798257702920650772").getAsMention();
//            } else if (rank < 32) {
//                rankHistory += Guill.jda.getEmoteById("798257702827982848").getAsMention();
//            } else if (rank < 48) {
//                rankHistory += Guill.jda.getEmoteById("798257702894960680").getAsMention();
//            } else if (rank < 64) {
//                rankHistory += Guill.jda.getEmoteById("798257705268674560").getAsMention();
//            } else if (rank < 80) {
//                rankHistory += Guill.jda.getEmoteById("798257702886834186").getAsMention();
//            } else if (rank == 80) {
//                rankHistory += Guill.jda.getEmoteById("798257702677118977").getAsMention();
//            }
    ...
    public static HashMap<String, String> nameChangeScrape(String playerName) {
    ...
    public static String guildScrape(String playerName) {
        ...
        try {
            ...
            if (doc.body().getElementsContainingText("Guild history is hidden").size() != 0)
                return "Guild history is hidden.";

            if (doc.body().getElementsContainingText("No guild changes detected.").size() != 0)
                return "No guild changes detected.";

            Elements profile = doc.body().getElementsByTag("table");

            if (profile.size() < 1) return "Could not detect guild changes.";
            ...
            for (Element nameChange : guildChangeTable.children().get(1).children()) {
                ...
                if (!lastGuildName.equals(name)) {
                    ...
                    if (rank.replaceAll(" ", "").isEmpty()) {
                        ...
                        if (guildHistory.length() < 950) guildHistory += name.trim() + ":\n↳ Rank: " + rank + " From: " + from + " To: " + (to.trim().isEmpty() ? "Now" : to) + "\n\n";
                    }
